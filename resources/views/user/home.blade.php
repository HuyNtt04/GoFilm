    ...
    <section class="relative h-screen w-full overflow-hidden">
        <div id="slider" class="w-full h-full flex slider-container transition-all duration-500 ease-in-out">
            ...
            <div class="w-full h-full relative flex-shrink-0">
                <img src="{{ asset($lastestMovie->thumbnail) }}" class="absolute inset-0 w-full h-full object-cover" />
        ...
        <!-- 🔘 Thumbnails old -->
        <div id="thumbnail" class="absolute bottom-6 right-6 z-30 flex space-x-3">
                    ...
                    <div class="relative group-carousel">
                        ...
                        <div class="overflow-hidden">
                            <div class="carousel-track flex transition-transform duration-500 ease-in-out">
                                ...
                                @for ($i = 0; $i < count($koreaMovies); $i++) <div
                                    class="min-w-[33.3333%] px-2 box-border">
                                    <div class="bg-[#1d1d1d] rounded-xl overflow-hidden shadow-lg">
                                        <img src="{{ asset($koreaMovies[$i]->thumbnail) }}"
                                            alt="{{ $koreaMovies[$i]->title }}" class="w-full h-[170px] object-cover" />
                                        <div class="p-2">
                    ...
                    <div class="overflow-hidden">
                        <div class="carousel-track flex transition-transform duration-500 ease-in-out">
                            ...
                            <div class="min-w-[33.3333%] px-2 box-border">
                                <div class="bg-[#1d1d1d] rounded-xl overflow-hidden shadow-lg">
                                    <img src="{{ asset($chinaMovie->thumbnail) }}" alt="{{ $chinaMovie->title }}"
                ...
                <div class="relative group-carousel">
                    ...
                    <div class="overflow-hidden">
                        <div class="carousel-track flex transition-transform duration-500 ease-in-out">
                            ...
                            @for ($i = 0; $i < count($animeMovies); $i++) <div class="min-w-[33.3333%] px-2 box-border">
                                <div class="bg-[#1d1d1d] rounded-xl overflow-hidden shadow-lg">
                                    <img src="{{ asset($animeMovies[$i]->thumbnail) }}"
                                        alt="{{ $animeMovies[$i]->title }}" class="w-full h-[170px] object-cover" />
                                    <div class="p-2">
    ...
    <div class="max-w-screen-xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="bg-[#1b1b1b] rounded-2xl p-4 border border-white/10">
            ...
            <ul class="space-y-3">
                ...
                @for ($i = 0; $i < count($recentHotMovie5); $i++) <a
                    href="{{ route('MovieInfo.show', $recentHotMovie5[$i]->id) }}">
        ...
        <div class="bg-[#1b1b1b] rounded-2xl p-4 border border-white/10">
            ...
            <ul class="space-y-3">
                @for ($i = 0; $i < count($recentFavoriteMovie5); $i++) <a
                    href="{{ route('MovieInfo.show', $recentFavoriteMovie5[$i]->id) }}">
                    <li class="pb-3 flex items-center gap-3 text-sm">
                        <span class="text-gray-400">{{ $i + 1 }}.</span>
                        <img src="{{ asset($recentFavoriteMovie5[$i]->thumbnail) }}" class="w-7 h-10 rounded" />
        ...
        <div class="bg-[#1b1b1b] rounded-2xl p-4 border border-white/10">
            ...
            <ul class="space-y-3">
                ...
                @for ($i = 0; $i < count($top5HotCategories); $i++) <li class="flex items-center gap-3 text-sm">
                    <a href="{{ route('MoviesByCategory.index', $top5HotCategories[$i]->id) }}">
                        <span class="text-gray-400">{{ $j = $i + 1 }}.</span>
                        <span class="px-2 py-0.5 rounded-full {{ $top5HotCategories[$i]->classColor }} text-xs">
                ...
                <div id="top10Track" class="flex transition-transform duration-500 ease-in-out">
                    ...
                    <div class="min-w-[20%] px-2 box-border">
                        <div class="relative group">
                            <span
                                class="absolute top-2 left-2 z-10 text-[3.5rem] font-bold text-yellow-400 drop-shadow-[0_0_6px_rgba(0,0,0,0.7)] font-[anton] leading-none"></span>
            ...
            <div class="hero-slide absolute inset-0 opacity-0 transition-opacity duration-700 ease-in-out">
                <div class="flex flex-col md:flex-row h-full">
                    <div class="w-full md:w-[45%] p-6 md:p-10 flex flex-col justify-center z-10">
                        ...
                        <button
                            class="bg-yellow-400 hover:bg-yellow-300 text-black font-bold px-5 py-2 rounded-full text-sm flex items-center gap-2 w-fit">
                            <i class="fa fa-play"></i> <a href="{{ route('MovieInfo.show', $lastestMovie->id) }}">Xem
                                ngay</a>
                        </button>
                    </div>
                    <div class="w-full md:w-[55%] relative rounded-r-2xl overflow-hidden">
                        <img src=" {{ asset($lastestMovie->thumbnail) }} " class="w-full h-full object-cover" />
                        <div class="absolute inset-0 bg-gradient-to-r from-black via-black/60 to-transparent">
                        </div>
                    </div>
                </div>
...
const slides = slider.querySelectorAll('div.flex-shrink-0'); // mỗi slide là 1 div flex-shrink-0
...
// Đảm bảo hiển thị đúng slide đầu tiên ban đầu
goToSlide(0);

// (Tuỳ chọn) Responsive: nếu resize thì cập nhật lại vị trí đúng slide đang chọn
window.addEventListener('resize', () => {
    const currentSlide = Math.round(Math.abs(parseInt(slider.style.transform.replace('translateX(-', '')) ||
        0) / slider.clientWidth);
